...
class TrainingCycleGAN(GANTrainingLoop):
    ...
    def _process_and_save_images(self, epoch, phase="validation", tuning_path=None):
        ...
        if "target_times" in data.keys():
            pred = self.Model.Generator(data["real_source"], data["target_times"], training=True).numpy()
        else:
            pred = self.Model.Generator(data["real_source"], training=True).numpy()
