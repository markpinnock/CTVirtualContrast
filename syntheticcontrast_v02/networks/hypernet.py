...
class LayerEmbedding(tf.keras.layers.Layer):
    ...
    def call(self, h):
        ...
        ks = tf.concat([tf.concat([tf.concat([h(self.z[i][j][k]) for k in range(self.out_kernels)], axis=4) for j in range(self.in_kernels)], axis=3) for i in range(self.depth_kernels)], axis=2)
